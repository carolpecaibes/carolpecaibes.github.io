---
layout: default
title: Cor
parent: Propriedades dos elementos
nav_order: 1
---

# Cor
{: .no_toc }

## Sumário
{: .no_toc .text-delta }

1. TOC
{:toc}

Todos os elementos podem ter sua cor modificada.

## Modificar a cor do elemento
1. Selecione o elemento
2. Selecione o ícone de paleta <img src="/assets/paleta.png" width="18">
3. Selecione a cor desejada de contorno e preenchimento ou especifique o código [HEX](https://www.google.com/search?q=seletor+cores+google&oq=seletor+cores+google&gs_lcrp=EgZjaHJvbWUyCQgAEEUYORiABDIHCAEQABjvBdIBCDQxMTlqMGo5qAIGsAIB8QW9PKfVwVFr0w&sourceid=chrome&ie=UTF-8) da cor desejada

Obs.: É possível escolher a opção de cor transparente selecionando a opção **Sem preenchimento**

## Modificar a opacidade da cor do elemento
1. Selecione o elemento
2. Selecione o ícone de engrenagem <img src="/assets/engrenagem.png" width="18">
3. Modifique a opacidade na barra deslizante correspondente.
